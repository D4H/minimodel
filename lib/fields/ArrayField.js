...
function ArrayField(descriptor, parent) {
  if (!(this instanceof ArrayField))
    return new ArrayField(descriptor, parent);

  Field.call(this, descriptor, parent);

  var arrayType = parent.__modelMetadata.options.schemaParser.parseField(descriptor.type[0]);
...
ArrayField.prototype.validate = function(done, options) {
  if(!this.value) {
    return done();
  }
...
ArrayField.prototype.setRaw = function(path, val) {
  ...
  if(!val) {
    ...
    val = path;
    self.value = val && val.map(function(arrVal) {
...
ArrayField.prototype._newItem = function() {
  return this.parent._instantiateType(this.arraySchema, this.arrayType, this.parent);
};
